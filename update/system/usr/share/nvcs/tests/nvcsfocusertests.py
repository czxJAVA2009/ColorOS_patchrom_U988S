...
class NvCSFocusPosTest(NvCSTestBase):
    ...
    def __init__(self, options):
        NvCSTestBase.__init__(self, "FocusPos")
    ...
    def runTest(self, args):
        ...
        if(totalSteps > 1024):
            self.logger.warning("Total focuser steps (%d) is larger than 1024" %
                                totalSteps)
        totalStepsTemp = totalSteps
        while (totalStepsTemp & 0x0001 == 0) and (totalStepsTemp != 0):
            totalStepsTemp = totalStepsTemp >> 1
            self.logger.info("totalStepsTemp = 0x%x" % totalStepsTemp)
        if totalStepsTemp != 1:
            self.logger.warning("Total focuser steps (%d) is not a power of 2" %
                                    totalSteps)
...
class NvCSSharpnessTest(NvCSTestBase):
    ...
    def runPreTest(self):
        ...
        self.obCamera.setAttr(nvcamera.attr_autofocus, 0)
